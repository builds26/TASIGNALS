"""Builds26 Signal Desk Bot — individual signals + daily digest edition."""
...
def format_daily_digest(open_signals, pair_candles):
    ...
    for symbol, sig in open_signals.items():
        ...
        # Distance to TP/SL (positive = closer to TP, negative = closer to SL)
        if direction == 'LONG':
        ...
        status = '✓' if sig.get('tp1_hit') else '·'
        msg += f"{emoji} <b>{symbol} {direction}</b>  ({age} old){' · TP1 ✓' if sig.get('tp1_hit') else ''}\n"
...
def should_send_digest(meta):
    """Check if it's time for the 08:00 UTC daily digest and we haven't sent today's yet."""
    now = datetime.now(timezone.utc)
    today_str = now.strftime('%Y-%m-%d')
    last_digest_date = meta.get('last_digest_date')
    if last_digest_date == today_str:
        return False
    # Send between 08:00 and 08:30 UTC window
...
def main():
    ...
    for p in PAIRS:
        try:
            ...
            # NEW signal: post if Strong AND not already tracked for this pair
    ...
    resolved_keys = []
        ...
        msg, verdict, pct = format_resolution(open_sig, status, hit_price, hit_ts)
        print(f"  RESOLVED {symbol}: {status} ({verdict}, {pct:+.2f}%)")
        # Add to history FIRST (so even if Telegram or commit fails, we won't repost)
        history.append({
            **open_sig,
            'status': status,
            'verdict': verdict,
