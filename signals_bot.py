"""
HWMF Signal Bot.
Posts a market snapshot of all 10 pairs every cycle with traffic lights.
"""
import os
import json
...
def send_telegram(text):
    ...
    try:
        with urllib.request.urlopen(req, timeout=20) as r:
            ...
            print(f"Telegram OK: message_id={result.get('result',{}).get('message_id')}")
            return result
...
def format_snapshot(rows):
    ...
    for r in rows:
        ...
        if sig['direction'] in ('LONG', 'SHORT') and sig['sl']:
            msg += f"   <i>SL ${fmt_price(sig['sl'])} · TP1 ${fmt_price(sig['tp1'])} · TP2 ${fmt_price(sig['tp2'])}</i>\n"
    msg += "\n<i>🟢 long · 🔴 short · 🟡 neutral</i>"
    return msg
